  ...
  return (
    <header className={`fixed top-0 w-full z-40 transition-all duration-400 ${scrolled ? 'glass-nav py-4' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
        ...
        >
          RA<span className="text-white/40">.</span>
        </motion.a>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link, i) => (
            <motion.a
              ...
              className="text-sm font-medium text-[#888] hover:text-white transition-colors font-mono"
            >
              <span className="text-white/30 mr-1 text-xs">0{i + 1}.</span>
              {link.name}
            </motion.a>
          ))}
          <motion.a
            ...
            <div className="flex flex-col items-center py-8 gap-5">
              {navLinks.map((link, i) => (
                <a key={link.name} href={link.href} onClick={() => setMobileMenuOpen(false)}
                  className="text-base font-medium text-[#888] hover:text-white font-mono transition-colors">
                  <span className="text-white/30 mr-2 text-xs">0{i + 1}.</span>{link.name}
                </a>
              ))}
